...
export const MacBookFrame: React.FC<FrameProps> = ({ children, className = '' }) => {
    return (
        <div className={`relative mx-auto ${className}`} style={{ maxWidth: '800px', aspectRatio: '16/9' }}>
            ...
            <div className="relative h-full w-full bg-gray-800 rounded-t-xl border-[4px] border-gray-300 shadow-2xl overflow-hidden ring-1 ring-gray-900/50">
                {/* Camera Notch Area */}
                <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-40 h-4 bg-gray-900 rounded-b-lg z-20 flex items-center justify-center">
            ...
            <div className="relative -mt-[2px] h-3 w-[110%] -ml-[5%] bg-gray-300 rounded-b-lg shadow-xl flex items-center justify-center border-t border-gray-400">
                <div className="w-20 h-1 bg-gray-400 rounded-full opacity-50"></div>
            </div>
        </div>
    );
};
